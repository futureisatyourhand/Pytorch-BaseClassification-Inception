...
class InceptionModule(nn.Module):
    def __init__(self,channels_in,channels_out):
        ...
        self.inception1_0=nn.Sequential(
            nn.Conv2d(channels_in,channels_out[0],1,1,bias=True),
            nn.BatchNorm2d(channels_out[0]),
            )
        self.inception1_1=nn.Sequential(
            nn.Conv2d(channels_in,channels_out[1],1,1,bias=True),
            nn.BatchNorm2d(channels_out[1]),
        ...
        self.inception1_2=nn.Sequential(
            ...
            nn.BatchNorm2d(channels_out[3]),
            #nn.Conv2d(channels_out[3],channels_out[4],5,1,2,bias=True),
            nn.Conv2d(channels_out[3],channels_out[4],3,1,1,bias=True),
            nn.BatchNorm2d(channels_out[4]),
            nn.Conv2d(channels_out[4],channels_out[4],3,1,1,bias=True),
            nn.BatchNorm2d(channels_out[4]),
        )
        self.inception1_3=nn.Sequential(
            nn.MaxPool2d(3,1,1),
            nn.Conv2d(channels_in,channels_out[5],1,1,bias=True),
            nn.BatchNorm2d(channels_out[5]),
        )
    def forward(self,x):
